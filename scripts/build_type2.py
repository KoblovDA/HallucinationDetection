"""Build augmented Type 2 dataset via LLM.

Same split as Type 1 / Type 3 (50 val / 150 test / rest train).
For train: N LLM calls per source with claim-diversity (excluding previous claims).
...
def process_source(triple: Triple, target_n: int, api_key: str) -> list[dict]:
    ...
    for variant_idx in range(target_n):
        prop = propose_type2(
            triple, api_key=api_key, exclude_claims=used_claims,
...
def main() -> int:
    ...
    parser.add_argument("--out-train", default="data/type2_train.jsonl")
    parser.add_argument("--out-val", default="data/type2_val.jsonl")
    parser.add_argument("--out-test", default="data/type2_test.jsonl")
    ...
    parser.add_argument("--progress", default="data/type2_progress.jsonl")
